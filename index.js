...
function handleTabClick(tabName) {
    ...
    document.querySelector(`a.${tabName}`).classList.add('activeLink');
}


// Scroll by a certain amount when a button is clicked
document.addEventListener('DOMContentLoaded', function() {
    const themeSelector = document.querySelector('.theme-selector');
    const scrollAmount = 100; // Adjust the scroll amount as needed

    // Add event listeners to buttons or other elements to control the scroll
    document.querySelectorAll('.scroll-button').forEach(button => {
        button.addEventListener('click', function() {
            // Scroll right when the button is clicked
            themeSelector.scrollBy({
                left: scrollAmount, // Scroll amount in pixels
    ...
    themeSelector.addEventListener('scroll', function() {
        console.log('Theme selector scrolled');
...
function applyTheme(themePath) {
  ...
  if (themePath.endsWith('.mp4') && videoBackground) {
    videoSource.src = themePath; // Update the video source
    videoBackground.load();      // Reload the video to apply the new source
    videoBackground.classList.remove('fade-out'); // Remove fade-out
    videoBackground.classList.add('fade-in');    // Add fade-in
...
//Note that if any applyTheme() function is already present make sure to delete or comment out the portion